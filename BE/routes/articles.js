...
const path = (app) => {
  app.get("/articles", (req, res) => {
    const q =
      "SELECT id_articles,a.titre,a.content,DATE_FORMAT(date_publication, '%W %e %M %Y') AS date_publication, auteur, m.description_media,m.url FROM articles a inner join medias m ON (a.id_medias= m.id_medias) order by id_articles desc";
    db.query(q, (err, data) => {
  ...
  app.post("/articles", authenticateToken, (req, res) => {
    ...
    if (!auteur) {
      res
        .status(400)
        .json({ error: "L'auteur est obligatoire" });
      return;
    }
    db.query(
      "INSERT INTO articles(titre, content, date_publication, auteur, id_users, id_medias) VALUES(?, ?, ?, ?, ?, ?)",
      [titre, content, date_publication, auteur, id_users, id_medias],
      (error, data) => {
        if (error) {
          console.error(error);
